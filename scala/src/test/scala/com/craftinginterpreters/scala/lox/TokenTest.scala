...
object TokenTest:
  ...
  def parseToken(str: String): Token =
    ...
    val literal = parts.lift(1).orNull
    val lexeme = parts.lift(2).orNull
    val line = parts.lift(3).map(_.toInt).getOrElse(-1)
    ...
    if !opts.ignoreLiteral then
      obtained.typ match
        ...
        case TokenType.NUMBER =>
          assertEquals(0, BigDecimal(obtained.literal.toString)
            .compare(BigDecimal(expected.literal.toString)))
        case _ =>
